...
/* struct FILE_info getFileInfo(std::string str)
{
    struct FILE_info fi;

    std::stringstream ss(str);

    std::getline(ss,fi.fileName , ',' );
    std::getline(ss,fi.server_IP , ',' );
    std::getline(ss,fi.server_port , ',' );
...
std::string preSaveData_config(int sockfd,char *buffer, int byteRecv)
{
    ...
    tmp.append(delimStart.append(std::string(remote_ip).append(" start\n")));   // ###S
    tmp.append(std::string(buffer,byteRecv));
    tmp.erase(tmp.length()-1);  // erase "\n" in char*
    tmp.append("\n");
    tmp.append(delimEnd.append(std::string(remote_ip).append(" end\n")));     // ###E
    return tmp;
}

/* std::ostream& operator << (std::ostream& os, const struct FILE_info fi)
{
    os << "fileName: " << fi.fileName << std::endl;
    os << "server_IP: " << fi.server_IP << std::endl;
    os << "server_port: " << fi.server_port << std::endl;
    return os;    
} */
// Master dinh ki se gui goi tin keepalive de kiem tra xem server con hoat dong khong
...
int main()
{
    /* int listen = create_tcp_socket(12345);
    HandleConnection(listen, serverSide); 
      */
    pthread_t threadID;
